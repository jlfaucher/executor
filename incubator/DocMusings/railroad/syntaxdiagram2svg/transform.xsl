...
Iterate over the <syntaxdiagram> and create one SVG file per diagram.
The name of the file is given by the title of the syntax diagram.
The syntax diagrams without title are not processed.
        ...
        <xsl:for-each select=".//fragment">
            ...
            <xsl:if test="$fragment_title" >
                <xsl:for-each select="*[1]"> <!-- The entry point is always the first child of the fragment -->
                    <xsl:variable name="filename" select="concat(helper:end_separator($OUTPUTDIR), $diagram_title, '-', $fragment_title, $SUFFIX)"/>
